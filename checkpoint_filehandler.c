...
static int WriteTree(CpTreeNodePtr curr_node, ssize_t offset, FILE *f) {
  ...
  LLIter it;
  ssize_t child_bytes_written = 0, name_len, num_children,
          num_attempts, self_content_length;
  int res;
...
static int WriteChildren(CpTreeNodePtr curr_node,
                         ...
                         ssize_t offset,
                         FILE *f) {
  ssize_t self_content_length, res,
  ...
  for (ssize_t i = 0; i < num_children; i++) {
    ...
    res = WriteTree(curr_child, offset + child_bytes_written, f);
